...
| `NEXT_PUBLIC_SITE_URL` | 代码默认回退为 `packages/website/src/lib/site-origin.ts` 中的 Vercel 地址；**Vercel** 另在 `packages/website/vercel.json` 的 `build.env` 注入同名变量，避免未在控制台配置时 sitemap/robots 仍指向旧域。绑域后请改控制台变量或修改 `vercel.json` 并 Redeploy。 |
...
**仓库内一键脚本**（与下面手写 `lighthouse` 等价，默认跑 `/zh-Hans`、`/en`、`/zh-Hant/roadmap`）：

```bash
cd packages/website
npm run lighthouse:cwv
# 或：SITE_URL=https://你的域 npm run lighthouse:cwv
```
