...
export function AgentsDemo() {
  ...
  if (loading) return <p className="text-sm text-muted">Loading agents...</p>;
  if (error) return <p className="text-sm text-red-300">{error}</p>;
  if (items.length === 0) return <p className="text-sm text-muted">No agents available.</p>;
        ...
        <li key={agent.id} className="rounded-xl border border-border bg-card p-4">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {agent.capabilities.slice(0, 4).map((cap) => (
              <span key={`${agent.id}-${cap.name}`} className="rounded border border-border px-2 py-1 text-xs text-muted">
